...
def build():
    ...
    # lineterminator is pinned because csv.writer defaults to CRLF, which makes the output
    # differ from every other generated file and shows up as a spurious diff in git and CI.
    ...
    for name in ("regions","templates","sources","diagnostics"):
        with open(os.path.join(ROOT, name+".json"), encoding="utf-8") as fh:
            bundle[name] = json.load(fh)
    diag = bundle["diagnostics"]
    if not diag.get("diagnostics") or not diag.get("sources"):
        print("ERROR: diagnostics.json is empty or malformed; run build_diagnostics.py",
              file=sys.stderr)
        return 1
    bad = [(d["id"], k) for d in diag["diagnostics"] for k in d.get("src", [])
           if k not in diag["sources"]]
    if bad:
        print("ERROR: diagnostics reference unknown sources: %s" % bad, file=sys.stderr)
        return 1
    ...
    cats = {}
    for d in diag["diagnostics"]: cats[d["cat"]] = cats.get(d["cat"],0)+1
    print("    diagnostics: %d tests (%s), %d cited sources"
